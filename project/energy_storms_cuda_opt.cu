...
__global__ void calcStorms(Storm* stormPtr, float *layerDevice, float *layer_copyDevice,
		int layer_size, float* maximumDevice , int* positionsDevice) {
...
int main(int argc, char *argv[]) {
    ...
    cudaMemCopy(layer_copyDevice, layer_copy, sizeof(float) * layer_size, cudaMemcpyHostToDevice);

    // result
    float* maximumDevice;
    int*   positionsDevice;
    cudaMalloc((void **)&maximumDevice, sizeof(float) * num_storms);
    cudaMalloc((void **)&positionsDevice,  sizeof(int) * num_storms);
